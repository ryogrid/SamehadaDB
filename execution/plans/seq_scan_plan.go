...
func NewSeqScanPlanNode(schema *table.Schema, predicate *execution.Expression, tableOID uint32, outputSchema *table.Schema) *SeqScanPlanNode {
	return &SeqScanPlanNode{schema, predicate, tableOID, nil}
}

func (p *SeqScanPlanNode) GetPredicate() *execution.Expression {
	return p.predicate
}
