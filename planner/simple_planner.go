		...
		for _, sfield := range pner.qi.SelectFields_ {
			...
			for _, existCol := range tgtTblColumns {
				if existCol.GetColumnName() == *colName {
					isOk = true
					outColDefs = append(outColDefs, column.NewColumn(*colName, existCol.GetType(), false, types.PageID(-1), existCol.GetExpr()))
					break
				}
			}
			if !isOk {
...
func (pner *SimplePlanner) MakeSelectPlanWithJoin() (error, plans.Plan) {
	...
	var scanPlanL plans.Plan
	{
		...
		for _, col := range tgtTblColumnsL {
			columns = append(columns, column.NewColumn(tblNameL+"."+col.GetColumnName(), col.GetType(), false, types.PageID(-1), col.GetExpr()))
		}
	...
	var scanPlanR plans.Plan
	{
		...
		for _, col := range tgtTblColumnsR {
			columns = append(columns, column.NewColumn(tblNameR+"."+col.GetColumnName(), col.GetType(), false, types.PageID(-1), col.GetExpr()))
		}
	...
	var filterOut *schema.Schema
	{
		...
		for _, colDef := range tgtTblColumnsL {
			col := column.NewColumn(tblNameL+"."+colDef.GetColumnName(), colDef.GetType(), false, types.PageID(-1), colDef.GetExpr())
			col.SetIsLeft(true)
			finalOutCols = append(finalOutCols, col)
		}
		for _, colDef := range tgtTblColumnsR {
			col := column.NewColumn(tblNameR+"."+colDef.GetColumnName(), colDef.GetType(), false, types.PageID(-1), colDef.GetExpr())
			col.SetIsLeft(false)
			finalOutCols = append(finalOutCols, col)
		}
		...
		if hasWhere {
			...
			if len(pner.qi.SelectFields_) == 1 && *pner.qi.SelectFields_[0].ColName_ == "*" {
				...
				for _, sfield := range pner.qi.SelectFields_ {
					...
					col := column.NewColumn(*tblName+"."+*colName, colDef.GetType(), false, types.PageID(-1), colDef.GetExpr())
					if *tblName == tblNameL {
						col.SetIsLeft(true)
					} else { // Right
						col.SetIsLeft(false)
...
func (pner *SimplePlanner) MakeCreateTablePlan() (error, plans.Plan) {
	...
	for _, cdefExp := range pner.qi.ColDefExpressions_ {
		columns = append(columns, column.NewColumn(*cdefExp.ColName_, *cdefExp.ColType_, false, types.PageID(-1), nil))
	}
