...
txn_map : unordered_map<TxnID, *Transaction> = {}
...
//   transaction_manager.global_txn_latch.RLock()
  ...
  if (enable_logging) {
    ...
    log_record &LogRecord(txn.GetTransactionId(), txn.GetPrevLSN(), LogRecordType::BEGIN)
...
*/
  ...
  if (enable_logging) {
    // TODO(student): add logging here
    log_record := &LogRecord(txn.GetTransactionId(), txn.GetPrevLSN(), LogRecordType::COMMIT)
    lsn := transaction_manager.log_manager.AppendLogRecord(&log_record)
    txn.SetPrevLSN(lsn)
...
*/
  ...
  if (enable_logging) {
    // TODO(student): add logging here
    log_record := &LogRecord(txn.GetTransactionId(), txn.GetPrevLSN(), LogRecordType::ABORT)
